...
jq  --arg FEED "$FEED_NAME" '.expression.children[0].value=$FEED' < test/searchTemplates/findErrorStreams.json \
...
STREAM_COUNT=$(jq '.pageResponse.length' < "$OUTPUT_FILE")
EXACT=$(jq '.pageResponse.exact' < "$OUTPUT_FILE")
...
echo "Summary. Number of streams per pipeline are as follows:"
 jq -r '.values[].meta.pipelineUuid' <  "$OUTPUT_FILE" | sort | uniq -c \
